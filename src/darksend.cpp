    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat DTC and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002 DTC per transaction on average.
            */
            if(r <= 20)
            {
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeDayTraderCoinAmount*COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if(!sessionFoundMasternode){
        ...
        LogPrintf("Submitting Darksend for %f DTC CENT - sessionTotalValue %d\n", fNtrnSubmitted, sessionTotalValue);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100 DTC+1 ( bit on if present )
    // bit 1 - 10 DTC+1
    // bit 2 - 1 DTC+1
    // bit 3 - .1 DTC+1
    // bit 3 - non-denom
...
void ThreadCheckDarkSendPool(void* parg)
{
    ...
    RenameThread("daytradercoin-darksend");
        ...
        if(c % (60*5)*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeDayTraderCoinAmount = (rand() % nLeftToAnon)+3;
                } else {
