...
void Shutdown(void* parg)
{
    ...
    RenameThread("DayTraderCoin-shutoff");
...
//        CTxDB().Close();
        ...
        MilliSleep(50);
        LogPrintf("DayTraderCoin exited\n\n");
        fExit = true;
#ifndef QT_GUI
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("DayTraderCoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  daytradercoind [options]                     " + "\n" +
                  "  daytradercoind [options] <command> [params]  " + _("Send command to -server or daytradercoind") + "\n" +
                  "  daytradercoind [options] help                " + _("List commands") + "\n" +
                  "  daytradercoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "DayTraderCoin:"))
                fCommandLine = true;
...
bool static InitError(const std::string &str)
{
    ...
    uiInterface.ThreadSafeMessageBox(str, _("DayTraderCoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("DayTraderCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: daytradercoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: daytradercoind.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 32001 or testnet: 25724)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 32101 or testnet: 25825)") + "\n" +
    ...
    strUsage += "  -anonymizeDayTraderCoinamount=<n> " + _("Keep N DayTraderCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  DayTraderCoin is probably already running."), strDataDir.c_str()));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("DayTraderCoin version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "DayTraderCoin server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("DayTraderCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of DayTraderCoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart DayTraderCoin to complete") << "\n";
    ...
    if(fMasterNode) {
        ...
        if(!strMasterNodeAddr.empty()){
            ...
            if (addrTest.GetPort() != GetDefaultPort())  {
                std::string errorMessage = strprintf("Invalid -masternodeaddr port %u detected in daytradercoin.conf (only %d is supported for mainnet)", addrTest.GetPort(), GetDefaultPort());
    ...
    nAnonymizeDayTraderCoinAmount = GetArg("-anonymizeDayTraderCoinamount", 0);
    if(nAnonymizeDayTraderCoinAmount > 999999) nAnonymizeDayTraderCoinAmount = 999999;
    if(nAnonymizeDayTraderCoinAmount < 2) nAnonymizeDayTraderCoinAmount = 2;
    ...
    LogPrintf("Anonymize DayTraderCoin Amount %d\n", nAnonymizeDayTraderCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1DayTraderCoin+1000 == (.1DayTraderCoin+100)*10
       10DayTraderCoin+10000 == (1DayTraderCoin+1000)*10
    */
