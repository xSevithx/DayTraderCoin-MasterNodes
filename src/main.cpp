...
static const int64_t nTargetTimespan = 20 * 60;  // DayTraderCoin - every 20mins
unsigned int nTargetSpacing = 1 * 79; // DayTraderCoin - 79 secs
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;

static const int64_t nDiffChangeTarget = 1;

unsigned int nStakeMinAge = 1 * 60 * 60; // DayTraderCoin - 5 hours
unsigned int nStakeMaxAge = 168 * 60 * 60; // DayTraderCoin - 5 hours
unsigned int nModifierInterval = 10 * 60; // DayTraderCoin - time to elapse before new modifier is computed
...
const string strMessageMagic = "DayTraderCoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    LogPrintf("CTxMemPool::accept() : accepted %s (poolsz %u)\n",
           hash.ToString().substr(0,10).c_str(),
           mapTx.size());
    return true;
}
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        if (!IsInitialBlockDownload()) {
            ...
            LogPrintf("ConnectBlock() : Is initial download, skipping masternode and developer payment checks %d\n", pindexBest->nHeight+1);
        }
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        strMiscWarning = strMessage;
        LogPrintf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DayTraderCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "02 December 2017";
        CTransaction txNew;
        txNew.nTime = 1512233999;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        if(!fTestNet)
            assert(block.hashMerkleRoot == uint256("0x645c4fcfb50838b247d8274366edcfc615f0b649edffe0cc97269fe58d8bdf4a"));
        else
            assert(block.hashMerkleRoot == uint256("0x645c4fcfb50838b247d8274366edcfc615f0b649edffe0cc97269fe58d8bdf4a"));
...
unsigned char pchMessageStart[4] = { 0x67, 0x34, 0xff, 0xcc };
